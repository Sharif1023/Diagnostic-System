...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Validate input
        $consumable_code = trim($_POST['consumable_code']);
        $name = trim($_POST['name']);
        $price = (float)$_POST['price'];
        $unit = trim($_POST['unit']);

        if (empty($consumable_code) || empty($name) || empty($unit)) {
        ...
        // Check if consumable code already exists
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM consumable_info WHERE consumable_code = ?");
        $stmt->execute([$consumable_code]);
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Consumable code already exists");
        }

        // Insert new consumable
        $stmt = $pdo->prepare("
            INSERT INTO consumable_info (
                consumable_code,
                name,
                price,
                unit
...
<head>
    ...
    <title>Add New Consumable - Laboratory Management System</title>
    <style>
        body { 
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="consumable_code" name="consumable_code" required
                           value="<?php echo isset($_POST['consumable_code']) ? htmlspecialchars($_POST['consumable_code']) : ''; ?>"
                           placeholder="e.g., NEEDLE_21G">
                </div>
                <div class="form-group">
